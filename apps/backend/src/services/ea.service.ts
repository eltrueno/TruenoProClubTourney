...
export async function searchClubs(query: string): Promise<{ clubId: string; name: string; regionId: number; customKit?: boolean; crestAssetId?: string }[]> {
  ...
  const list = (results ?? []) as unknown as {
    clubId: string;
    clubInfo: { name: string; regionId: number; teamId: number; customKit?: { crestAssetId?: string, dCustomKit?: string } };
  }[];
  return list.map((r) => ({
    clubId: r.clubId,
    name: r.clubInfo.name,
    regionId: r.clubInfo.regionId,
    teamId: r.clubInfo.teamId,
    customKit: Number(r.clubInfo.customKit?.dCustomKit) > 0,
